...
const engines=await import(process.env.PLAYWRIGHT_MODULE||'@playwright/test');const out='.verification/results';const rows=[];const postId=JSON.parse(fs.readFileSync(out+'/browser.json')).find(r=>r.name==='wp71').postId;
for(const name of ['chromium','firefox','webkit']){
 const b=await engines[name].launch({headless:true,args:name==='chromium'?['--no-sandbox']:[]});let r={browser:name,postId};const ctx=await b.newContext({viewport:{width:1440,height:1100}});const p=await ctx.newPage();
 try{
  await p.goto('http://localhost:8895/?p='+postId);r.loaded=await p.evaluate(async()=>{const f=await document.fonts.load('24px "Noto Sans Japanese"','日本語');return f.length>0&&f.every(x=>x.status==='loaded')});assert.ok(r.loaded);await p.screenshot({path:out+'/'+name+'-public.png'});await p.reload();r.cachedReload=await p.evaluate(async()=>{await document.fonts.ready;return [...document.fonts].some(f=>f.family.replace(/["']/g,'')==='Noto Sans Japanese'&&f.status==='loaded')});assert.ok(r.cachedReload);
  await ctx.route('https://fonts.raspi0124.dev/**',route=>route.abort());await p.reload();r.outageFontRejected=await p.evaluate(async()=>{try{await document.fonts.load('24px "Noto Sans Japanese"','日本語');return false}catch{return true}});assert.ok(r.outageFontRejected);assert.ok((await p.locator('body').innerText()).includes('日本語'));r.outageReadable=await p.locator('.wp-block-tinyjpfont-noto').last().evaluate(e=>e.getBoundingClientRect().height>0);assert.ok(r.outageReadable);await p.screenshot({path:out+'/'+name+'-outage.png'});
