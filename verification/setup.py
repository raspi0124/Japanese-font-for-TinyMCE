...
for name,version,image,port in matrix:
 ...
 services[name]={'image':image,'entrypoint':['apache2-foreground'],'ports':['127.0.0.1:'+str(port)+':80'],'volumes':[str(OUT/name/'wordpress')+':/var/www/html',str(ROOT)+':/var/www/html/wp-content/plugins/japanese-font-for-tinymce:ro',str(ROOT/'verification/bootstrap.php')+':/tmp/bootstrap.php:ro'],'environment':{'AUDIT_URL':'http://localhost:'+str(port),'AUDIT_PASSWORD':password},'depends_on':{'db-'+name:{'condition':'service_healthy'}}}
