...
const out='.verification/results/dev5.00_6';fs.mkdirSync(out,{recursive:true});
const rows=[];
try { for(const site of JSON.parse(fs.readFileSync('.verification/matrix.json'))){
 ...
 const text=await news.innerText();assert.ok(text.includes('5.00-dev.5から'));assert.ok(text.includes('保存しているサイト'));assert.ok(text.includes('古いWordPress'));
 const response=await p.request.get(await news.locator('a').getAttribute('href'));assert.equal(response.status(),200);assert.ok((await response.text()).includes('= 5.00-dev.6 ='));
